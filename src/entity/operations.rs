...
impl<'a> EntityOpHost<'a> {
  ...
  pub fn eq(&self, analyzer: &Analyzer<'a>, lhs: Entity<'a>, rhs: Entity<'a>) -> Option<bool> {
    if let Some(true) = self.strict_eq(analyzer, lhs, rhs) {
      return Some(true);
    }
  ...
  pub fn strict_eq(
    ...
    if let (Some(lhs_lit), Some(rhs_lit)) = (lhs_lit, rhs_lit) {
      ...
      return if all_neq { Some(false) } else { None };
    }
  ...
  pub fn binary_op(
    ...
    let to_result = |result: Option<bool>| factory.boolean_maybe_unknown(result);
