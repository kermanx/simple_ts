...
impl<'a> EntityTrait<'a> for ArgumentsEntity<'a> {
  ...
  fn get_property(&'a self, _analyzer: &mut Analyzer<'a>, _key: Entity<'a>) -> Entity<'a> {
    unreachable!()
  }
  ...
  fn construct(&'a self, _analyzer: &mut Analyzer<'a>, _args: Entity<'a>) -> Entity<'a> {
    unreachable!()
  }
  ...
  fn iterate(&'a self, analyzer: &mut Analyzer<'a>) -> IteratedElements<'a> {
    ...
    for (spread, entity) in &self.arguments {
      if *spread {
        if let Some(iterated) = entity.iterate_result_union(analyzer) {
