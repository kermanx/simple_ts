...
impl<'a> ScopeContext<'a> {
  pub fn new(factory: &EntityFactory<'a>) -> Self {
    ...
    cf.push(CfScope::new(CfScopeKind::Module, None, Some(false)));
...
impl<'a> Analyzer<'a> {
  ...
  pub fn push_call_scope(
    ...
    let call_id = self.scope_context.call_id_counter;
    self.scope_context.call_id_counter += 1;
